...
namespace westcoast_cars.web.Controllers
{
    ...
    public class TransmissionsController : Controller
    {
        ...
        public async Task<IActionResult> Create(TransmissionTypePostViewModel model)
        {
            ...
            if (await _context.TransmissionTypes.SingleOrDefaultAsync(c => c.Name.ToUpper() == model.Name.ToUpper()) is not null)
            {
            ...
            await _context.TransmissionTypes.AddAsync(transmission);
        ...
        {
            var transmissions = await _context.TransmissionTypes
                .OrderBy(c => c.Name)
